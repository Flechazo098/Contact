...
public class NeoForgeMailboxDataProvider implements IMailboxDataProvider {

    private PlayerMailboxData getData() {
        var server = DataManager.getServer();
        var overworld = server.getLevel(Level.OVERWORLD);
        return overworld.getData(ContactAttachments.MAILBOX_DATA);
    }

    public NeoForgeMailboxDataProvider() {
    }
