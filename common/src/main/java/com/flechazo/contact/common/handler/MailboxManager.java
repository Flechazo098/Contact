...
public final class MailboxManager {
    ...
    public static void onServerTick(MinecraftServer server) {
        ...
        if (updateTick == 0) {
            for (MailToBeSent mail : PlatformHelper.getMailList()) {
                mail.tick(20);
                if (mail.isReady()) {
                    var uuid = mail.getUUID();
                    if (PlatformHelper.addMailboxContents(uuid, mail.getContents())) {
            ...
            if (!READY_TO_REMOVE.isEmpty()) {
                PlatformHelper.getMailList().removeAll(READY_TO_REMOVE);
    ...
    public static void updateState(UUID uuid, PlayerMailboxData data) {
        var posData = data.getMailboxPos(uuid);
        if (posData != null) {
            updateState(DataManager.getServer().getLevel(posData.dimension()), posData.pos());
        }
    }
